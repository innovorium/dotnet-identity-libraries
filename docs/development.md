...
- PostgreSQL 18.4 for the provider integration tests
...
Provider integration tests run when `INNOVORIUM_TEST_POSTGRES` contains a
PostgreSQL connection string. CI and release builds supply a disposable
PostgreSQL 18.4 service. Without that variable, local database tests are
reported as skipped; a local run is not equivalent to the complete CI gate.
